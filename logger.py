...
class Tacotron2Logger(SummaryWriter):
    ...
    def log_training(self, reduced_loss, grad_norm, learning_rate, duration,
                     iteration, diagonality=None, avg_prob=None):
            if not (diagonality is None):
                self.add_scalar("training.attention_alignment_diagonality", diagonality, iteration)
            if not (avg_prob is None):
    ...
    def log_validation(self, reduced_loss, model, y, y_pred, iteration, diagonality, avg_prob):
        ...
        idx = 2 # and randomly pick a second one to plot
        self.add_image(
            "alignment3",
            plot_alignment_to_numpy(alignments[idx].data.cpu().numpy().T),
            iteration, dataformats='HWC')
        ...
        idx = 3 # and randomly pick a second one to plot
        self.add_image(
            "alignment4",
            plot_alignment_to_numpy(alignments[idx].data.cpu().numpy().T),
            iteration, dataformats='HWC')
